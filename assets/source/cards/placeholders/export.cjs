...
(async()=>{
 ...
 for(let i=0;i<samples.length;i++){
  ...
  if(m){
   ...
   layers.push({input:Buffer.from(`<svg width="590" height="860"><g font-family="serif" font-size="70" font-style="italic" text-anchor="middle" fill="#090703"><text x="158" y="807">${m.atk}</text><text x="430" y="807">${m.def}</text></g></svg>`),left:0,top:0});
  }
