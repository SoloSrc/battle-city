...
(async()=>{
 ...
 fs.writeFileSync(path.join(review,'index.html'),`<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SOLOSRC · 72 card placeholders</title><style>body{background:#252329;color:#eee;font:16px system-ui;margin:32px}header{max-width:850px}h1{font-size:28px}input,select{padding:10px;background:#403b42;color:white;border:1px solid #887c82;margin:8px}main{display:grid;grid-template-columns:repeat(auto-fill,minmax(190px,1fr));gap:28px}article{min-width:0}h2{font-size:14px;max-width:230px}p{color:#c6bfc5}article p,article a{font-size:12px}a{color:#e7c895}.card{position:relative;width:177px;height:258px}.card img{position:absolute}.art{left:5.932%;top:4.651%;width:88.136%;height:65.116%;object-fit:contain;background:#28232b}.frame{inset:0;width:100%;height:100%}.stars{position:absolute;left:8.814%;top:74.419%;display:flex;gap:.6px}.stars img{position:static;width:9.6px;height:9.6px}.attr{left:83.051%;top:73.488%;width:8.136%;height:5.581%}.atk,.def{position:absolute;top:84.42%;font:700 16.8px Georgia;color:#272019}.atk{left:11.525%}.def{left:57.288%}[hidden]{display:none}</style><header><h1>72 original card placeholders</h1><p>Symbolic silhouettes for gameplay testing. These are not final illustrations. Labels and motif notes belong to this review page only.</p><p>The approved frame is unchanged. Preview fit is selectable because systems §6.2 leaves the runtime square-art crop policy undecided. Spell/trap badges are omitted here; this page is not CardView or a shader acceptance test.</p><label>Find card <input id="search" type="search"></label><label>Preview fit <select id="fit"><option value="contain">Contain square (default)</option><option value="cover">Cover window (preview only)</option></select></label></header><main>${html.join('')}</main><script>document.querySelector('#search').oninput=e=>document.querySelectorAll('article').forEach(a=>a.hidden=!a.dataset.name.includes(e.target.value.toLowerCase()));document.querySelector('#fit').onchange=e=>document.querySelectorAll('.art').forEach(a=>a.style.objectFit=e.target.value);</script></html>`);
 ...
 for(let i=0;i<samples.length;i++){
  ...
  const art=await sharp(path.join(out,id+'.png')).resize(520,560,{fit:'contain',background:'#28232b'}).png().toBuffer();
