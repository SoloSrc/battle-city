...
(async()=>{
 ...
 for(const f of fs.readdirSync(src).filter(x=>x.endsWith('.svg'))){
  ...
  if(frame){for(let y=40;y<690;y++)for(let x=35;x<555;x++)if(data[(y*590+x)*4+3]!==0)throw Error(stem+' window has opaque pixels '+x+','+y);}
  if(stem==='card_back'){
   ...
   if(total/data.length>0.2)throw Error('Back rotational symmetry mean error '+total/data.length); // Subpixel antialiasing can differ along matching edges.
  }
