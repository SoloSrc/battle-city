...
| Character clips | `assets/characters/anims/character_anims.glb` | `idle` (4 s) and `walk` (1 s) clips, `footstep_l` / `footstep_r` call-method events on walk; played through the `AnimationTree` state machine |
...
- glTF Binary (`.glb`), **+Y up**, 1 unit = 1 m, scale 1.0, all transforms
  applied. Characters face −Z in Godot (−Y in Blender before export).
- Origin at the feet for characters and standing props; at the bounds-min
  corner for kit pieces that tile on the 1 m grid.
- Suffixes: `<name>-col` trimesh collision (mesh hidden), `-convcol`
  convex, `-noimp` skipped, `-navmesh` navigation source.
- Animation: NLA tracks as separate actions, 30 fps, always sample, no
  ...
  the `anim_` prefix). Events are call-method tracks named per
  systems.md §3.2.
